// categories.js — Decorafit FF&E · Taxonomia compartilhada
//
...
function decorafitNormalize(raw) {
  return String(raw || '')
    .toLowerCase()
    .normalize('NFD')
    .replace(/[̀-ͯ]/g, '')
...
function decorafitMapSiteCategory(raw) {
  if (!raw) return null;
  const c = decorafitNormalize(raw);
...
function decorafitCategoryFromName(name) {
  const n = decorafitNormalize(name);
...
function decorafitGuessCategory(name, siteCategory) {
  const mapped = decorafitMapSiteCategory(siteCategory);
  if (mapped) return mapped;
  return decorafitCategoryFromName(name);
}
